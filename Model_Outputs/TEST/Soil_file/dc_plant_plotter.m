...
DNS_depth_inds = depth_ind_finder(variable_names, 'DNS_RT_');

% Get the variable names for 'NH4' and 'NO3' depth data
DNS_depth_vars = variable_names(DNS_depth_inds);

% Concatenate data for 'NH4' and 'NO3' variables along the depth dimension
DNS_depth = cat_depth(fname, DNS_depth_vars);


z = zeros(1,19);
N = size(DNS_depth,2); % amount of depth layers
fac = 2/3;
z(1:N) = 0.01*exp((1:N)*fac) / exp(fac);

ind = logical(std(DNS_depth));
