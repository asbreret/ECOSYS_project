...
for i = 2:size(Matrix, 1)
    ...
    for j = 1:size(Matrix, 2)
        if isnan(Matrix(i, j)) && ~isnan(last_val)
            Matrix(i, j) = last_val;
        else
            last_val = Matrix(i, j);
        end
...
% Set up the new depth grid and interpolate
N = 20;
Matrix(1,17) = N-1;
z_new = 0.01 + 3.49 * ((0:(N-1)) / (N-1)).^2;
Matrix_new = NaN(size(Matrix, 1), N);
Matrix_new(1:2, :) = [Matrix(1, :); z_new];
z_old = Matrix(2, :)+1;


for i = 3:size(Matrix, 1)
    valid_indices = ~isnan(Matrix(i, :)) & z_old ~= 0;
    [unique_z, unique_idx] = unique(z_old(valid_indices));
    unique_data = Matrix(i, valid_indices(unique_idx));
    Matrix_new(i, :) = interp1(unique_z, unique_data, z_new, 'linear','extrap');
    
    % If it's the third row, set NaNs to zero
    if i == -1
        Matrix_new(i, isnan(Matrix_new(i, :))) = 0.00000001;
    else
        % Find the first non-NaN number in the row
        first_valid_data = find(~isnan(Matrix_new(i, :)), 1, 'first');
        M = first_valid_data;

        if ~isempty(first_valid_data)
            % Fill NaNs in the top meter with the first non-NaN value
            Matrix_new(i, isnan(Matrix_new(i, 1:M))) = Matrix_new(i, first_valid_data);
        end
    end
end
